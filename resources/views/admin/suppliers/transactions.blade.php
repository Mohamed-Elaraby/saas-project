...
@section('title', __('trans.supplier transactions'))
                    ...
                    <div class="card custom-card">
                        ...
                        <div class="card-header">
                            <div class="card-title">
                                {{ __('trans.supplier transactions') }}
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="bd-example mb-3">
                                <div class="d-flex justify-content-center mb-3">
                                    <div class="p-2">{{ $supplier -> name }}</div>
                                    <div class="p-2">{{ __('trans.balance') .'  '.number_format($supplier -> balance, 2).'  ' . \App\Traits\SettingTrait::getSetting()->currency }}</div>
                                    <div class="p-2">
                                        @if (Auth::user()->hasPermission('create-supplierPayment'))
                                            <a class="btn btn-outline-primary btn-sm" href="{{ route('admin.supplierPayments.create', ['supplier_id' => $supplier->id]) }}">{{ __('trans.pay')}}</a>
                                        @endif

                                        @if (Auth::user()->hasPermission('create-supplierCollecting'))
                                            <a class="btn btn-outline-primary btn-sm" href="{{ route('admin.supplierCollecting.create', ['supplier_id' => $supplier->id]) }}">{{ __('trans.collect')}}</a>
                                        @endif
                                    </div>
                                </div>
                                <div class="d-flex justify-content-center mb-3">
                                ...
                                <div class="d-flex justify-content-center mb-3">
                                    ...
                                    <div class="p-2">
                                        <button
                                            ...
                                            data-link="{{ route('admin.supplierTransactions.print', ['supplier_id' => $supplier->id]) }}"
...
<script>
    ...
    $('#filter_date_button').on('click', function () {
        ...
        let table = $('#suppliertransaction-table');
    ...
    $('#print_date_button').on('click', function() {
        ...
        let start_date = $('.start_date').val();
        let end_date = $('.end_date').val();
        let search = $('#suppliertransaction-table_filter input[type="search"]').val();

        printJS("{{ route('admin.supplierTransactions.print', ['supplier_id' => $supplier->id]) }}" + '?start_date=' + start_date + '&end_date=' + end_date + '&search=' + search);
    })
