...
<head>
    ...
    <title>{{ __('trans.receipt of payment') . ' ' . __('trans.to supplier') . ' ' . $payment_data -> supplier -> name }}</title>
...
<div class="buttons_area d-grid gap-2">
    ...
    <a href="{{ route('admin.supplierPayments.index') }}" class="btn btn-warning btn-sm ">
        <span>{{ __('trans.all supplier payments') }} <i class="ri-arrow-go-back-line"></i></span>
    </a>

    <a href="{{ route('admin.supplierTransactions', $payment_data -> supplier -> id) }}" class="btn btn-info btn-sm ">
...
<section class="receipt-container">
                    ...
                    <td width="50%">
                        <table class="table_no_border">
                            <tr>
                                ...
                                <td width="80%">: {{ $payment_data -> branch -> display_name }}</td>
                            </tr>
                            <tr>
                                <th width="20%">{{ __('trans.phone') }}</th>
                                <td width="80%">: {{ $payment_data -> branch -> phone }}</td>
                            </tr>
                            @if($payment_data -> branch -> vat_number)
                                <tr>
                                    <th width="20%">{{ __('trans.vat number') }}</th>
                                    <td width="80%">: {{ $payment_data -> branch -> vat_number }}</td>
                                </tr>
                            @endif
                        </table>
                    </td>
    ...
    <main>
        <h3 style="text-align: center">{{ __('trans.receipt of payment') . ' ' . __('trans.to supplier') . ' ' . $payment_data -> supplier -> name }}</h3>
        <table style="width: 100%" class="details_table">
            <tr>
                <th style="width: 30%">{{ __('trans.amount paid') }}</th>
                <td style="width: 70%">{{ $payment_data -> amount_paid + $payment_data -> amount_paid_bank .' '. \App\Traits\SettingTrait::getSetting()->currency}} </td>
            </tr>
            <tr>
                ...
                <td style="width: 70%">
                    @if($payment_data -> payment_method && $payment_data -> payment_method_bank)
                        ...
                        {{  __('trans.part') .' '. $payment_data -> payment_method  .' '.   __('trans.and part') .' '.  $payment_data -> payment_method_bank }}
        ...
        <table class="table_no_border" style="width: 100%">
            <tr>
                <td style="width: 50%; text-align: center"><h4>{{ __('trans.receiver signature') }}</h4></td>
