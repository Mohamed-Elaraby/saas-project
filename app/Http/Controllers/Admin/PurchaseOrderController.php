...
class PurchaseOrderController extends Controller
{
    ...
    {
        $this->middleware(['permission:read-purchaseOrder', 'checkOrdersStatus'])->only('index');
        $this->middleware('permission:create-purchaseOrder')->only('create');
        $this->middleware('permission:update-purchaseOrders')->only('edit');
        $this->middleware('permission:delete-purchaseOrders')->only('destroy');
    }
...
//        dd($request->all());
        ...
        $data_except_amount_paid = $request -> except(['amount_paid', 'amount_paid_bank', 'search_terms', 'soft_save', 'final_save', 'product_data']);
...
//                $total_vat = $request -> total_vat  ? $request -> total_vat  : null;
...
//        $purchaseOrderData = $request ->except(['product_data']);
        $data_except_amounts = $request -> except(['amount_paid', 'amount_paid_bank', 'search_terms', 'soft_save', 'final_save', 'product_data']);
        ...
        $payment_method_bank = $request -> payment_method_bank ? $request -> payment_method_bank : null;
        // if amount paid request empty set amount paid equal zero
        $amount_paid = $request->amount_paid ? $request->amount_paid : 0;
...
//            dump([$new_order_product_code => $new_order_product_quantity]);
            $product_sold = PurchaseOrderProduct::where(['purchase_order_id' => $purchaseOrder -> id, 'item_code' => $new_order_product_code]) -> first();
        ...
        foreach ($sold_products_list as $product)
        {
            ...
            if (!in_array($product_sold_code, $new_products_codes_array)) {
                ...
                PurchaseOrderProduct::where(['purchase_order_id' => $purchaseOrder -> id, 'item_code' => $product_sold_code]) -> delete();
