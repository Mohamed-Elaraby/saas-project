...
use App\Models\MoneySafeOpeningBalance;
...
//            $date = $date_range;
            if (!empty($start_date) && !empty($end_date) && !empty($branch_id)){
                ...
                $money_safe_opening_balance                         = MoneySafeOpeningBalance::where('branch_id', $branch_id) -> whereDate('updated_at', $start_date)->first('balance') -> balance ?? 0;
...
//    }
    ...
    public function statement_print(Request $request)
    {
        ...
        $data['statements']                                         = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->get();
        $data['total_imports_cash']                                 = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('imports_cash');
        $data['total_imports_network']                              = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('imports_network');
        $data['total_imports_bank_transfer']                        = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('imports_bank_transfer');
        $data['total_card_details_hand_labour']                     = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('card_details_hand_labour');
        $data['total_card_details_new_parts']                       = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('card_details_new_parts');
        $data['total_card_details_used_parts']                      = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('card_details_used_parts');
        $data['total_card_details_tax']                             = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('card_details_tax');
        $data['total_expenses_cash']                                = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('expenses_cash');
        $data['total_expenses_network']                             = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('expenses_network');
        $data['total_custody_administration_cash']                  = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('custody_administration_cash');
        $data['total_custody_administration_network']               = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('custody_administration_network');
        $data['total_cash_to_administration']                       = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('cash_to_administration');
        $data['total_advances_and_salaries_cash']                   = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('advances_and_salaries_cash');
        $data['total_advances_and_salaries_network']                = Statement::where('branch_id', $branch_id) -> whereBetween('created_at', $date_range)->sum('advances_and_salaries_network');

        $total_imports                                      = ($data['total_imports_cash'] + $data['total_imports_network'] + $data['total_imports_bank_transfer']) ;
        $total_card_details                                 = $data['total_card_details_hand_labour'] + $data['total_card_details_new_parts'] + $data['total_card_details_used_parts'] + $data['total_card_details_tax'] ;
        $total_expenses                                     = $data['total_expenses_cash'] + $data['total_expenses_network'] ;
        $total_custody_administration                       = $data['total_custody_administration_cash'] + $data['total_custody_administration_network'] ;
        $total_advances_and_salaries                        = $data['total_advances_and_salaries_cash'] + $data['total_advances_and_salaries_network'] ;
        $money_safe_opening_balance                         = MoneySafeOpeningBalance::where('branch_id', $branch_id) -> whereDate('updated_at', $start_date)->first('balance') -> balance ?? 0;
        $total_bank_transfer_and_network                    = $data['total_imports_network'] + $data['total_imports_bank_transfer'];
        $current_balance                                    = $money_safe_opening_balance + $data['total_imports_cash'] - $total_expenses + $total_custody_administration - $data['total_cash_to_administration'] - $total_advances_and_salaries;

        $data['total_imports_formatted']                            = $this->format_number($total_imports);
        $data['total_card_details_formatted']                       = $this->format_number($total_card_details);
        $data['total_expenses_formatted']                           = $this->format_number($total_expenses);
        $data['total_custody_administration_formatted']             = $this->format_number($total_custody_administration);
        $data['total_advances_and_salaries_formatted']              = $this->format_number($total_advances_and_salaries);

        $data['total_imports_cash_formatted']                       = $this->format_number($data['total_imports_cash']);
        $data['total_imports_network_formatted']                    = $this->format_number($data['total_imports_network']);
        $data['total_imports_bank_transfer_formatted']              = $this->format_number($data['total_imports_bank_transfer']);
        $data['total_card_details_hand_labour_formatted']           = $this->format_number($data['total_card_details_hand_labour']);
        $data['total_card_details_new_parts_formatted']             = $this->format_number($data['total_card_details_new_parts']);
        $data['total_card_details_used_parts_formatted']            = $this->format_number($data['total_card_details_used_parts']);
        $data['total_card_details_tax_formatted']                   = $this->format_number($data['total_card_details_tax']);
        $data['total_expenses_cash_formatted']                      = $this->format_number($data['total_expenses_cash']);
        $data['total_expenses_network_formatted']                   = $this->format_number($data['total_expenses_network']);
        $data['total_custody_administration_cash_formatted']        = $this->format_number($data['total_custody_administration_cash']);
        $data['total_custody_administration_network_formatted']     = $this->format_number($data['total_custody_administration_network']);
        $data['total_cash_to_administration_formatted']             = $this->format_number($data['total_cash_to_administration']);
        $data['total_advances_and_salaries_cash_formatted']         = $this->format_number($data['total_advances_and_salaries_cash']);
        $data['total_advances_and_salaries_network_formatted']      = $this->format_number($data['total_advances_and_salaries_network']);

        $data['money_safe_opening_balance_formatted']               = $this->format_number($money_safe_opening_balance);
        $data['total_bank_transfer_and_network_formatted']          = $this->format_number($total_bank_transfer_and_network);
        $data['current_balance_formatted']                          = $this->format_number($current_balance);
