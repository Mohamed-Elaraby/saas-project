...
class ClientPaymentController extends Controller
{
    ...
    {
        $this->middleware('permission:read-clientPayment')->only('index');
        $this->middleware('permission:create-clientPayment')->only('create');
        $this->middleware('permission:update-clientPayment')->only('edit');
        $this->middleware('permission:delete-clientPayment')->only('destroy');
    }
...
//        /* Update Client balance */
        ...
        $this -> insertToClientTransaction(
            ...
            [
                ...
                'details'                       => __('trans.payment document') . ' ' . $clientPayment -> receipt_number,
//                'details'                       => 'سند صرف رقم / ' . $clientPayment -> receipt_number,
        ...
        Statement::create([
            ...
            'notes'                             => $clientPayment -> notes ? __('trans.financial custody from the administration') . ' ( ' . $clientPayment -> notes. ' )' : __('trans.financial custody from the administration'),
            'branch_id'                         =>  $request -> branch_id,
        ]);
        ...
        $this -> insertToStatement(
            ...
            [
                ...
                'notes'                         =>  __('trans.payment document to customer') . ' ' . $clientPayment -> client -> name,
//                'notes'                         =>  ' سند صرف الى العميل ' . $clientPayment -> client -> name,
                'branch_id'                     =>  $request -> branch_id,
            ]
        );
        toast(__('trans.client payment created successfully'),'success')->background('#000');
        return redirect()->route('admin.clientPayments.show', $payment_id);
    }
