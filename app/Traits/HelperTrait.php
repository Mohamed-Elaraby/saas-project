...
trait HelperTrait
{
    ...
    public function uploadSingleImageAndRemoveOldImage($_photoName = null, $_personalDefine, $_photoType, $_personalName, $_diskName, $_photoWidth, $_photoHeight, $_oldDataObject = null)
    {
        if ($_photoName){  // This Condition Use On Create And Update Case
            ...
            if ( $this -> old_image_path != null)
            {
                // Check profile picture Of Doctor Exists And Not Default Image
                if (Storage::disk($this -> disk_name) ->exists($this -> old_image_path)){
                    // Remove Picture And Folder For The Doctor Until Doctor Name Folder
                    Storage::disk($this -> disk_name) ->deleteDirectory($this -> old_image_path);
